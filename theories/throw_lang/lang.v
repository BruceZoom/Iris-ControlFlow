...
(* err < cont < brk < ret *)
(* Inductive ek_prior : exit_kind -> exit_kind -> Prop :=
| cont_err : ek_prior ek_cont ek_err
| brk_err : ek_prior ek_brk ek_err
| ret_err : ek_prior ek_ret ek_err
| brk_cont : ek_prior ek_brk ek_cont
| ret_cont : ek_prior ek_ret ek_cont
| ret_brk : ek_prior ek_ret ek_brk. *)
...
Definition of_sval (sv : sval) : expr :=
  match sv with 
  | SVal v => Val v
  | SThrow v => Throw $ Val v
  end.

Definition to_sval (e : expr) : option sval :=
  match e with
  | Val v => Some $ SVal v
  | Throw (Val v) => Some $ SThrow v
  | _ => None
  end.
...
Inductive is_cf_terminal : expr -> Prop :=
  | throw_is_cft v: is_cf_terminal $ Throw (Val v).
...
Proof. solve_decision. Defined.
(* Instance ek_eq_dec : EqDecision exit_kind.
Proof. solve_decision. Defined. *)
...
Qed.
(* Instance ek_countable : Countable exit_kind.
Proof.
    refine (inj_countable' (λ ek, match ek with
    ek_cont => 0 | ek_brk => 1 | ek_ret => 2 | ek_err => 3 end)
    (λ n, match n with 0 => ek_cont | 1 => ek_brk | 2 => ek_ret | _ => ek_err end) _); by intros [].
Qed. *)
...
Proof.
 set (enc :=
   fix go e :=
     ...
     | Throw ek msg => GenNode 21 [ek; gov msg]
     | Catch ek => GenNode 22 [ek]
     end
   with gov v :=
     match v with
 ...
 set (dec :=
   fix go e :=
     ...
     | GenNode 21 [ek; msg] => Throw ek (gov msg)
     | GenNode 22 [ek] => Catch ek
...
Fixpoint comp_ectx (K1 K2 : ectx) : ectx :=
  ...
  (* MARK: *)
  | ThrowCtx K => ThrowCtx (comp_ectx K K2)
  | CatchCtx K=> CatchCtx (comp_ectx K K2)
  end.

(* MARK: impenetrable contexts *)
Inductive impenetrable_ectx : expr -> ectx -> Prop :=
  | ThrowImpenCatch e K:
    impenetrable_ectx (Throw e) (CatchCtx K)
  | CompImpenCtx e K K':
    ~ K' = EmptyCtx ->
...
Fixpoint fill (K : ectx) (e : expr) : expr :=
  ...
  | FaaRCtx e1 K => FAA e1 (fill K e)
  (* MARK: *)
  | ThrowCtx K => Throw (fill K e)
  | CatchCtx K => Catch (fill K e)
  end.
...
Fixpoint subst (x : string) (v : val) (e : expr)  : expr :=
  ...
  (* MARK: new rules for new expressions *)
  | Throw e => Throw (subst x v e)
  | Catch e => Catch (subst x v e)
  end.
...
Inductive head_step : expr → state → list observation → expr → state → list expr → Prop :=
  ...
  | CatchValS v σ:
    head_step (Catch (Val v)) σ [] (Val v) σ []
  | CatchThrowS v σ:
    head_step (Catch (Throw (Val v))) σ [] (Val v) σ []
  | CFCtxS e K σ:
...
Proof.
  destruct 1; clear H0.
  apply val_head_stuck in H1. subst.
...
(* Lemma fill_step_inv K e1 σ1 κ e2 σ2 efs :
  to_val e1 = None → prim_step (fill K e1) σ1 κ e2 σ2 efs →
  ∃ e3, e2 = fill K e3 ∧ prim_step e1 σ1 κ e3 σ2 efs.
Proof.
  intros Hnval [K'' e1'' e2'' Heq1 -> Hstep].
  destruct (step_by_val K K'' e1 e1'' σ1 κ e2'' σ2 efs) as [K' ->]; eauto.
...
Proof.
  ...
  - right; exists (ThrowCtx K1); eauto.
  - right; exists (CatchCtx K1); eauto.
Qed.

Lemma throw_penetrable_preservation v K σ1 κ e2 σ2 efs:
  ¬ impenetrable_ectx (Throw $ Val v) K ->
  prim_step (fill K (Throw $ Val v)) σ1 κ e2 σ2 efs ->
  σ1 = σ2 /\ κ = [] /\ efs = [] /\
  exists K', e2 = fill K' (Throw $ Val v) /\ ¬ impenetrable_ectx (Throw $ Val v) K'.
Proof.
  intros.
  inversion H0; subst.

  pose proof fill_cf_inv _ _ _ _ (throw_is_cft _) H1 as [[v' ?] | [K1 ?]]; subst;
  ...
  pose proof cf_step_congruence _ _ _ _ _ _ _ (throw_is_cft _) H1 H3 as [? [? [? ?]]]; subst.
  repeat split; auto.
  exists K0.
  split; auto.
Qed.
...
Definition outer (K : ectx) : ectx :=
  ...
  (* MARK: new rules for new contexts *)
  | ThrowCtx _ => ThrowCtx EmptyCtx
  | CatchCtx _ => CatchCtx EmptyCtx
  end.
...
Proof.
  ...
  destruct (to_sval (fill K (Throw $ Val v))) eqn:eq.
  - destruct K; simpl in eq; inversion eq.
    + exists nil, (Val v), σ, nil.
      apply Ectx_step with EmptyCtx (fill (CatchCtx EmptyCtx) (Throw $ Val v)) (Val v); auto.
      constructor.
    + destruct_inversion K H2.
  - replace (fill (CatchCtx K) (Throw $ Val v)) with (fill (CatchCtx EmptyCtx) (fill K (Throw $ Val v))); auto.
    replace (CatchCtx K) with (comp_ectx (CatchCtx EmptyCtx) K) in H0; auto.
...
Proof.
  ...
  + destruct_inversion (fill K e2) eq;
    [exfalso | destruct_inversion e eq].
